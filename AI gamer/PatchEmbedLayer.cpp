...
PatchEmbedLayer::PatchEmbedLayer(cudnnHandle_t cudnnHandle, cublasHandle_t cublasHandle, int batchSize, int inC, int inH, int inW, int patchSize, int embedDim, const char* layerName, bool train, float weightDecay, int gradAccumLength, WeightInitMethod weightInitMethod) :
	...
	if(train_){
		...
		CUDAMallocZero(&gradClassToken_, embedDim_*sizeof(__half));
		...
		CUDAMallocZero(&m_ClassToken_, embedDim_*sizeof(__half));
		CUDAMallocZero(&v_ClassToken_, embedDim_*sizeof(__half));
	}
}
...
void PatchEmbedLayer::UpdateParameters(float lr){
	...
	if(useAdamW_){
		...
		AdamWHalf(classToken_, gradClassToken_, m_ClassToken_, v_ClassToken_, lr, t_, 0.0f, embedDim_);
	} else{
		SGDHalf(weights_, gradWeights_, weightCount_, lr, weightDecay_);
		SGDHalf(posEmbed_, gradPosEmbed_, posCount_, lr, 0.0f);
		SGDHalf(classToken_, gradClassToken_, embedDim_, lr, 0.0f);
	}
	++t_;
}
void PatchEmbedLayer::SaveParameters(std::ofstream& file, unsigned char* buffer){
	...
	cudaMemcpy(buffer, classToken_, embedDim_*sizeof(__half), cudaMemcpyDeviceToHost);
...
void PatchEmbedLayer::LoadParameters(std::ifstream& file, unsigned char* buffer){
	...
	cudaMemcpy(classToken_, buffer, embedDim_*sizeof(__half), cudaMemcpyHostToDevice);
}
void PatchEmbedLayer::SaveOptimizerState(std::ofstream& file, unsigned char* buffer){
	...
	file.write(reinterpret_cast<const char*>(buffer), posCount_*sizeof(__half));
	cudaMemcpy(buffer, m_ClassToken_, embedDim_*sizeof(__half), cudaMemcpyDeviceToHost);
	file.write(reinterpret_cast<const char*>(buffer), embedDim_*sizeof(__half));
	cudaMemcpy(buffer, v_ClassToken_, embedDim_*sizeof(__half), cudaMemcpyDeviceToHost);
	file.write(reinterpret_cast<const char*>(buffer), embedDim_*sizeof(__half));
...
void PatchEmbedLayer::LoadOptimizerState(std::ifstream& file, unsigned char* buffer){
	...
	file.read(reinterpret_cast<char*>(buffer), embedDim_*sizeof(__half));
	cudaMemcpy(m_ClassToken_, buffer, embedDim_*sizeof(__half), cudaMemcpyHostToDevice);
	file.read(reinterpret_cast<char*>(buffer), embedDim_*sizeof(__half));
	cudaMemcpy(v_ClassToken_, buffer, embedDim_*sizeof(__half), cudaMemcpyHostToDevice);
	file.read(reinterpret_cast<char*>(&t_), sizeof(int));
}
size_t PatchEmbedLayer::GetParameterSize(){ return (weightCount_ + posCount_ + embedDim_)*sizeof(__half); }
size_t PatchEmbedLayer::GetOptimizerStateSize(){ return useAdamW_ ? (weightCount_ + posCount_ + embedDim_)*sizeof(__half)*2 + sizeof(int) : 0; }
