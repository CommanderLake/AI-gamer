...
PatchEmbedLayer::PatchEmbedLayer(cudnnHandle_t cudnnHandle, cublasHandle_t cublasHandle, int batchSize, int inC, int inH, int inW, int patchSize, int controlTokens, int controlInputDim, int embedDim, const char* layerName, bool train, float weightDecay, int gradAccumLength, WeightInitMethod weightInitMethod) :
	cudnn_(cudnnHandle), cublas_(cublasHandle), batchSize_(batchSize), inC_(inC), inH_(inH), inW_(inW), patchSize_(patchSize), embedDim_(embedDim), controlTokens_(controlTokens), controlInputDim_(controlInputDim), weightDecay_(weightDecay), gradAccumLength_(gradAccumLength){
...
__half* PatchEmbedLayer::Forward(__half* data){
	...
	if(controlInputCount_>0){
		...
		if(controlWeights_ && controlInput_){
			checkCUBLAS(cublasGemmEx(cublas_, CUBLAS_OP_N, CUBLAS_OP_N, embedDim_, batchControlCount_, controlInputDim_, &alpha_, controlWeights_, CUDA_R_16F, embedDim_, controlInput_, CUDA_R_16F, controlInputDim_, &beta0_, controlOut, CUDA_R_16F, embedDim_, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
			if(controlBiases_){ AddBias(controlOut, controlBiases_, embedDim_, batchControlCount_); }
		} else{
			checkCUDA(cudaMemset(controlOut, 0, controlOutputCount_*sizeof(__half)));
		}
	}
	return outData_;
}
__half* PatchEmbedLayer::Backward(__half* grad){
	...
	if(controlInputCount_>0 && controlWeights_ && controlInput_ && gradControlWeights_){
		const auto* gradControl = grad + static_cast<size_t>(batchSize_)*featureSize_;
		checkCUBLAS(cublasGemmEx(cublas_, CUBLAS_OP_N, CUBLAS_OP_T, embedDim_, controlInputDim_, batchControlCount_, &controlAlphaWeights_, gradControl, CUDA_R_16F, embedDim_, controlInput_, CUDA_R_16F, controlInputDim_, betaWeights, gradControlWeights_, CUDA_R_16F, embedDim_, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
		if(controlBiases_ && gradControlBiases_){
			AccumulateBiasGrad(gradControl, gradControlBiases_, embedDim_, batchControlCount_, controlAlphaWeights_, resetAccum);
		}
	}
...
size_t PatchEmbedLayer::GetParameterSize(){
	...
	return (weightCount_ + posCount_ + controlWeightCount + controlBiasCount)*sizeof(__half);
}
size_t PatchEmbedLayer::GetOptimizerStateSize(){
	if(!useAdamW_) return 0;
	const size_t controlWeightCount = controlWeights_ ? static_cast<size_t>(controlInputDim_)*embedDim_ : 0;
	const size_t controlBiasCount = controlBiases_ ? static_cast<size_t>(embedDim_) : 0;
	return (weightCount_ + posCount_ + controlWeightCount + controlBiasCount)*sizeof(__half)*2 + sizeof(int);
}
