...
PatchEmbedLayer::PatchEmbedLayer(cudnnHandle_t cudnnHandle, cublasHandle_t cublasHandle, int batchSize, int inC, int inH, int inW, int patchSize, int embedDim, const char* layerName, bool train, float weightDecay, int gradAccumLength) :
...
__half* PatchEmbedLayer::Forward(__half* data){
	inData_ = data;
	ExtractPatches(data, patchBuffer_, batchSize_, inC_, inH_, inW_, patchSize_, patchRows_, patchCols_);
...
__half* PatchEmbedLayer::Backward(__half* grad){
	...
	CombinePatchGrads(outGrad_, gradPatchBuffer_, batchSize_, inC_, inH_, inW_, patchSize_, patchRows_, patchCols_);
	return outGrad_;
}
void PatchEmbedLayer::UpdateParameters(float lr){
	if(accumCount_ % gradAccumLength_ > 0) return;
	if(useAdamW_){ AdamWHalf(weights_, gradWeights_, m_Weights_, v_Weights_, lr, t_, weightDecay_, weightCount_); } else{ SGDHalf(weights_, gradWeights_, weightCount_, lr, weightDecay_); }
	++t_;
}
...
size_t PatchEmbedLayer::GetParameterSize(){ return weightCount_*sizeof(__half); }
size_t PatchEmbedLayer::GetOptimizerStateSize(){ return weightCount_*sizeof(__half) + sizeof(int); }
