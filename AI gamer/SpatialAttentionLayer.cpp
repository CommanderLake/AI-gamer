...
SpatialAttentionLayer::SpatialAttentionLayer(cudnnHandle_t cudnnHandle, int attentionChannels, int numHeads, int batchSize, int channels, int height, int width, const char* layerName, bool train, float weightDecay) : cudnnHandle_(cudnnHandle),
	...
	if(train_){
		WeightInit(keyWeights_, dwWeightSizeAll_, 9*channelsPerHead_, channelsPerHead_, Xavier);
		WeightInit(queryWeights_, dwWeightSizeAll_, 9*channelsPerHead_, channelsPerHead_, Xavier);
		WeightInit(valueWeights_, pwWeightSizeAll_, channelsPerHead_, attCPerHead_, Xavier);
