...
#include "LeakyReLU.h"
#include "Sigmoid.h"
CustomOutLayer::CustomOutLayer(cudnnHandle_t cudnnHandle, cublasHandle_t cublasHandle, cudnnTensorDescriptor_t outDesc, int batchSize, int inputSize, int hiddenSize, const char* layerName, bool train): cudnnHandle_(cudnnHandle), cublasHandle_(cublasHandle),
	batchSize_(batchSize){
	layerName_ = layerName;
	train_ = train;
	outDesc_ = outDesc;
	constexpr auto wdButs = 0.00001f;
	constexpr auto wdaxes = 0.00001f;
	...
	buttonLayers_.push_back(new FCLayer(cudnnHandle_, cublasHandle_, batchSize_, hiddenSize, numButs_, "Buttons FC 1", train_, wdButs));
	buttonLayers_.push_back(new Sigmoid(buttonLayers_.back()->outDesc_, numButs_, batchSize_, "Buttons Sigmoid 0"));
	...
	axisLayers_.push_back(new FCLayer(cudnnHandle_, cublasHandle_, batchSize_, hiddenSize, numAxes_, "Axes FC 1", train_, wdaxes));
...
__half* CustomOutLayer::Forward(__half* data){
	...
	checkCUDA(cudaMemcpy(outData_, buttonData, numButs_*batchSize_*sizeof(__half), cudaMemcpyDeviceToDevice));
	checkCUDA(cudaMemcpy(outData_ + numButs_*batchSize_, axisData, numAxes_*batchSize_*sizeof(__half), cudaMemcpyDeviceToDevice));
