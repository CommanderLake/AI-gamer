...
#undef min
#undef max
NN::NN(cudnnHandle_t cudnnHandle, cublasHandle_t cublasHandle, int w, int h, bool train): cudnn_(cudnnHandle), cublas_(cublasHandle), batchSize_(80), seqLength_(train ? 4 : 1), gradAccumLength_(1){
	...
	layers_.push_back(new PatchEmbedLayer(cudnn_, cublas_, batchStateTotal_, seqLength_, 3, netHeight, netWidth, patchSize, embedDim, "PatchEmbed", train, wd, gradAccumLength_, Xavier));
