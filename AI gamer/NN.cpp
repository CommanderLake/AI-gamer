...
#undef min
#undef max
NN::NN(cudnnHandle_t cudnnHandle, cublasHandle_t cublasHandle, int w, int h, bool train) : cudnn_(cudnnHandle), cublas_(cublasHandle), batchSize_(4), gradAccumLength_(20){
	...
	//layers_.push_back(new CustomOutLayer(cudnn_, cublas_, batchSize_, tokensWithCls, embedSize, "SpatialActionHead", train, wd, gradAccumLength_));
