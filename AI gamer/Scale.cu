...
__global__ void ScaleNearestNeighborForwardKernel(const __half* input, __half* output, int batch, int channels, int inHeight, int inWidth, int outHeight, int outWidth, float scaleY, float scaleX){
...
__global__ void ScaleNearestNeighborBackwardKernel(const __half* gradOut, __half* gradIn, int batch, int channels, int inHeight, int inWidth, int outHeight, int outWidth, float scaleY, float scaleX){
	const size_t idx = blockIdx.x*blockDim.x + threadIdx.x;
	const size_t total = static_cast<size_t>(batch)*channels*outHeight*outWidth;
	if(idx >= total) return;
	const int outX = idx % outWidth;
