...
PatchMergingLayer::PatchMergingLayer(const cudnnHandle_t cudnnHandle, const cublasHandle_t cublasHandle, const int batchSize, const int tokens, const int embedDim, const int patchRows, const int patchCols, const char* layerName, const bool train, const float weightDecay, const int gradAccumLength, const WeightInitMethod weightInitMethod)
	...
	if(tokens_ != patchRows_*patchCols_){
		throw std::invalid_argument("PatchMergingLayer tokens must match patch grid");
	}
	if(patchRows_ % 2 != 0 || patchCols_ % 2 != 0){
		throw std::invalid_argument("PatchMergingLayer requires even patch rows/cols");
	}
...
size_t PatchMergingLayer::GetParameterSize(){
	return std::max(norm_->GetParameterSize(), reduction_->GetParameterSize());
}

size_t PatchMergingLayer::GetOptimizerStateSize(){
	return std::max(norm_->GetOptimizerStateSize(), reduction_->GetOptimizerStateSize());
}
