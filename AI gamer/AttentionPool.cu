...
namespace{
	...
	__host__ __device__ inline int NextPow2Clamp(int value, int limit){
		if(value <= 0){ return 1; }
		int pow2 = 1;
		while(pow2 < value && pow2 < limit){ pow2 <<= 1; }
		return pow2;
	}
}
__global__ void AttentionPoolScoresKernel(const __half* input, const __half* query, float* scores, int batchSize, int tokens, int embedDim, float invSqrtDim){
	...
	for(int c = 0; c < embedDim; ++c){
		sum = fmaf(__half2float(inputRow[c]), __half2float(query[c]), sum);
	}
...
void AttentionPoolForward(const __half* input, const __half* query, __half* output, float* attnWeights, float* tempBuffer, int batchSize, int tokens, int embedDim, float invSqrtDim){
	...
	const int maxThreads = static_cast<int>(maxTPB);
	int softmaxThreads = NextPow2Clamp(tokens, maxThreads);
	softmaxThreads = std::max(1, softmaxThreads);
	softmaxThreads = std::min(softmaxThreads, maxThreads);
