...
void WeightInit(__half* weights, const int elementCount, const int fanIn, const WeightInitMethod method){
	if(fanIn <= 0){
	...
	ConvertFloatToHalfScale(weights, weightFloat, elementCount, sqrtf(factor / fanIn));
