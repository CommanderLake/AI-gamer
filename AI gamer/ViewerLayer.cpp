...
ViewerLayer::ViewerLayer(int dataSize, const int channels, const int patchHeight, const int patchWidth, const int gridWidth, const std::string windowTitle, const float scale, bool backwardPass, __half* displayData) : displayData_(displayData), windowTitle_(windowTitle), backwardPass_(backwardPass), inC_(channels), inH_(patchHeight), inW_(patchWidth), gridW_(gridWidth), scale_(scale){
	outNCHW_ = dataSize;
...
void ViewerLayer::DisplayData(const __half* data){
	FeatureMapMosaic(displayData_ == nullptr ? data : displayData_, mosaicD_, inH_, inW_, inC_, mosaicDimW_, inW_, inH_, gridW_, scale_);
