...
#include "LayerNorm.h"
#include <algorithm>
PatchExpandingLayer::PatchExpandingLayer(const cudnnHandle_t cudnnHandle, const int batchSize, const int tokens, const int embedDim, const int patchRows, const int patchCols, const std::string layerName, const bool train, const float weightDecay, const int gradAccumLength,
	...
	expansion_ = new FCLayer(batchSize_ * tokens_, embedDim_, outEmbedDim_ * 4, "PatchExpandLinear", train, weightDecay, gradAccumLength, weightInitMethod, false);
