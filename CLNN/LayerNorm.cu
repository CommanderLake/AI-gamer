...
__global__ void LayerNormForwardFusedKernel(__half* __restrict__ y, const __half* __restrict__ x, const float* __restrict__ g, const float* __restrict__ b, float* __restrict__ mean, float* __restrict__ var, int N, int C, int HW){
	...
	if(warpId == 0){
		...
		if(laneId == 0){
			sharedMean = blockData.mean;
			const float variance = blockData.count > 0.0f ? blockData.m2/blockData.count : 0.0f;
			sharedVar = fmaxf(variance, 0.0f);
			mean[n] = sharedMean;
			var[n] = sharedVar;
		}
	}
	__syncthreads();
	const float invStd = rsqrtf(sharedVar + EPSILON_LN);
	for(int i = tid; i < stride; i += blockDim.x){
		const int c = i/HW;
		const float v = __half2float(xn[i]);
		const float norm = (v - sharedMean)*invStd;
		const float gamma = __ldg(g + c);
		const float beta = __ldg(b + c);
		y[n*stride + i] = __float2half(fmaf(norm, gamma, beta));
	}
}
__global__ void LayerNormForwardSpatialFusedKernel(__half* __restrict__ y, const __half* __restrict__ x, const float* __restrict__ g, const float* __restrict__ b, float* __restrict__ mean, float* __restrict__ var, int N, int C, int HW){
...
void LayerNormForward(__half* y, const __half* x, const float* g, const float* b, float* mean, float* var, int N, int C, int HW, bool spatialMode){
	...
	if(spatialMode){
		...
		LayerNormForwardFusedKernel<<<N, threads, smemSize>>>(y, x, g, b, mean, var, N, C, HW);
		checkCUDA(cudaGetLastError());
	}
}
__global__ void GradGammaBetaKernel(const __half* __restrict__ dy, const __half* __restrict__ x, const float* __restrict__ mean, const float* __restrict__ var, float* __restrict__ dG, float* __restrict__ dB, int N, int C, int HW){
	...
	for(int n = blockIdx.y; n < N; n += gridDim.y){
		...
		const float invStd = rsqrtf(varSafe + EPSILON_LN);
